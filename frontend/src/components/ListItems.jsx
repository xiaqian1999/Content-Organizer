...
const ListItems = ({id, company_name, title, application_url, required_skill, optional_skill, salary_range, year_of_experience, locations, rate_interest, additional_note, jobpost_url, fetchList}) => {
  return (
    <div id={id} className='w-full h-[300px] m-auto shadow-md overflow-y-scroll hover:shadow-xl bg-white p-2' style={{ transition: '0.3s', animation: 'fadeIn 1s'}}>
        <div className='px-2 rounded-t text-lg font-semibold cursor-pointer'>
            <a href={application_url} target="_blank">{company_name}</a>
            <p>{title}</p>
        </div>
